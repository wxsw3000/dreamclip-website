// MCP-Base SuperAdmin Control Console Core JS (Single Enterprise Base)
...
function switchTab(tabId, el) {
  ...
  const titles = {
    'tab-dashboard': '<span>📊</span> 平台总览大盘',
...
async function loadDashboard() {
  ...
  // 渲染大盘微服务简表
  const svcRes = await api('/microservices?size=5');
    ...
    if (svcRes.data.records.length === 0) {
      ...
      tbody.innerHTML = svcRes.data.records.map(s => `
        <tr>
          ...
          <td>
            ${s.docs_url ? `<a href="${s.base_url + s.docs_url}" target="_blank" style="color:#2563eb; text-decoration:none; font-weight:600;">📖 文档 ↗</a>` : '-'}
          </td>
...
async function loadUsers() {
  ...
  tbody.innerHTML = res.data.records.map(u => `
    <tr>
      ...
      <td><strong>${u.real_name}</strong></td>
      <td>${u.is_superadmin ? '<span class="badge badge-danger">👑 超级管理员</span>' : '<span class="badge badge-outline">操作员</span>'}</td>
      <td>${u.roles.map(r => `<span class="badge badge-tech">${r.role_name}</span>`).join(' ') || '-'}</td>
      <td><span class="badge ${u.status === 'ACTIVE' ? 'badge-success' : 'badge-danger'}">${u.status}</span></td>
      <td>${u.created_at ? u.created_at.replace('T', ' ').substring(0, 19) : '-'}</td>
      <td>
        <button class="btn btn-outline btn-sm" onclick="showToast('角色权限已与底层数据库关联打通', 'info')">⚙️ 权限</button>
      </td>
    </tr>
  `).join('');
}
...
function openUserModal() {
  showToast("可通过 API /api/v1/system/users 快速添加新系统操作员", "info");
}
...
async function loadConfigs() {
  ...
  tbody.innerHTML = res.data.map(c => `
    <tr>
      ...
      <td><input type="text" class="form-control" style="width:260px; display:inline-block;" value="${c.config_value}" id="cfg_${c.id}"></td>
